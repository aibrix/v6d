...
class EvictingCacheMap {
 private:
  ...
  typedef boost::intrusive::unordered_set<
      Node, boost::intrusive::hash<KeyHasher>,
 ...
 public:
  typedef std::function<void(std::vector<TPair>&&)> PruneHookCall;
  ...
  class iterator_base
   ...
   public:
    ...
    template <typename V, typename I,
              std::enable_if_t<std::is_same<V const, Value>::value &&
 ...
 private:
  ...
  struct KeyHasher {
    KeyHasher(const THash& keyHash)  // NOLINT(runtime/explicit)
        : hash(keyHash) {}
  ...
  struct KeyValueEqual {
    KeyValueEqual(const TKeyEqual& keyEqual)  // NOLINT(runtime/explicit)
        : equal(keyEqual) {}
   ...
   */
  void pruneWithFailSafeOption(std::size_t pruneSize, PruneHookCall pruneHook,
                               bool failSafe) {
    auto& ph = (nullptr == pruneHook) ? pruneHook_ : pruneHook;

    std::vector<TPair> pairs;
    pairs.reserve(pruneSize);
    for (std::size_t i = 0; i < pruneSize && !lru_.empty(); i++) {
      ...
      index_.erase(index_.iterator_to(*node));
      pairs.emplace_back(std::move(nptr->pr));
    }
