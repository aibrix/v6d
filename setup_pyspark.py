...
def find_pyspark_packages(root):
    pkgs = []
    for pkg in find_packages(root):
        if 'contrib.pyspark' in pkg:
            pkgs.append(pkg)
    return pkgs
...
setup(
    name='vineyard-pyspark',
    author='The vineyard team',
    author_email='developers@v6d.io',
    description='Vineyard integration with PySpark',
    long_description=long_description,
    long_description_content_type='text/x-rst',
    url='https://v6d.io',
    package_dir={'vineyard.contrib.pyspark': 'python/vineyard/contrib/pyspark'},
    packages=find_pyspark_packages('python'),
    cmdclass={'bdist_wheel': bdist_wheel_plat, "install": install_plat},
    zip_safe=False,
    install_requires=['vineyard', 'pyspark[complete]', 'click<8.1'],
